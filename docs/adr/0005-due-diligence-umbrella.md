...
- **Only structural rejection blocks the stochastic valuation.** `allows_stochastic` is `True` for every verdict except `rejected_for_stochastic`. Negative cash / funding gap / liquidity pressure is reported as a diagnostic but never blocks — blocking it would defeat the purpose of studying plan robustness under uncertainty.
